...
#SGC 4D is study with control (VS) triangular gridline
#SGC 4A is study with control vs sparse vs grid 
sgc4d <- df_subjects %>% filter(condition %in% c(111,113)) %>% droplevels()
sgc4a <- df_subjects %>% filter(condition %in% c(111,114,115)) %>% droplevels()

sgc4d_items <- df_items %>% filter( subject %in% sgc4d$subject) %>% droplevels()
...
title = "Participants by Condition and Data Collection Period"
cols = c("Orth-Full","Tri-Sparse","Total for Period")
cont <- table(sgc4d$term, sgc4d$condition)
cont %>% addmargins() %>% kbl(caption = title, col.names = cols) %>%  kable_classic()


title = "Participants by Condition and Data Collection Period"
cols = c("Orth-Full","Orth-Sparse","Orth-Grid","Total for Period")
cont <- table(sgc4a$term, sgc4a$condition)
cont %>% addmargins() %>% kbl(caption = title, col.names = cols) %>%  kable_classic()
...
## A — ORTH GRID \| OVERALL TASK  ACCURACY
...
df_i = sgc4a_items %>% filter(q %nin% c(6,9)) %>% 
