...
Because the graph comprehension task utilizes a Multiple-Response (MR) format (rather than simple multiple choice), the raw response data (the combination of answer options selected) for each question first need to be assigned a score. Approaches to scoring MR data and score transformations are derived in [Section -@sec-SGC5A-scoring].

### Hypothesis Testing

Experimental hypotheses are tested in [Section -@sec-SGC5A-hypotesting].

### Exploratory Analysis

Further data analyses are documented in [Section -@sec-SGC5A-exploration].
