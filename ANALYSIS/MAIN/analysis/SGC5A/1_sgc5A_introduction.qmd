---
subtitle: 'Study SGC5A | 1 Introduction'
---

\newpage

# Introduction {#sec-SGC5A-introduction}

**In Study 5A we explore the extent to which requiring mouse-cursor interaction with the graph improves interpretation of the underlying coordinate system. **

+------------------------------------+-----------------------------------------------------------------------------------------------------------+
| ![](/analysis/utils/img/11111.png) | **CONTROL (Check-box)**\                                                                                            |
|                                    | Demo: [111](https://limitless-plains-85018.herokuapp.com/?study=SGC5A&condition=11111&session=WEB-DEMO)   |
+------------------------------------+-----------------------------------------------------------------------------------------------------------+
| ![](/analysis/utils/img/11115.png) | **INTERACTION (Point-Click)**\                                                                                          |
|                                    | Demo: [11115](https://limitless-plains-85018.herokuapp.com/?study=SGC5A&condition=11115&session=WEB-DEMO) |
+------------------------------------+-----------------------------------------------------------------------------------------------------------+

: SGC5A Study Conditions
...
#IMPORT DATA 
df_subjects <- read_rds('analysis/SGC5A/data/0-study-level/sgc5_participants.rds') 
...
cont %>% addmargins() %>% kbl(caption = title) %>%  kable_classic()
...
_We hypothesize that requiring interaction with the graph (by clicking on data points rather than checkboxes to answer the questions) will improve interpretation of the coordinate system._

-   H1 The INTERACTION condition should yield significantly better performance
    -   H1A \| INTERACTION condition should yield higher total absolute score
    -   H1B \| INTERACTION condition should yield higher first question score
    -   H1C \| INTERACTION condition should yield lower response times on the first question (both overall, and specifically among participants who answer correctly)

**Null Hypothesis:**  
_A null effect will be characterized by no significant differences in performance between CONTROL and INTERACTION conditions._
...
### Data Preparation {#sec-SGC5-harmonize}
...
|----------------------------|--------------------|
| winter2022_clean_sgc5a.Rmd | 2_sgc5_scoring.qmd |
...
#import file
df_subjects <- read_rds("analysis/SGC5A/data/0-study-level/sgc5_participants.rds") #use RDS file as it contains metadata
...
#read datafiles
df_items <- read_rds("analysis/SGC5A/data/0-study-level/sgc5_items.rds") #use RDS file as it contains metadata
