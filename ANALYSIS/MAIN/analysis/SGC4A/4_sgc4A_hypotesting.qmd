...
m <- read_rds(file = "analysis/SGC4A/models/sgc4a_glmer_acc_mm.CrSQ.rds")
...
p <- df_i %>% 
  ...
  theme(legend.position = "bottom")+
   labs(title = "DISTRIBUTION | Question Interpretation",
       ...
       subtitle="")
p
# ggsave(p, filename = "figures/SGC4A_GRID_interpretation.png", width = 6, height =4)
...
Bmm.cat.rSQ <- brm( state ~ 1 + (1|subject) + (1|q), 
                 ...
                 # backend = "cmdstanr",
                 file ="analysis/SGC4A/models/sgc4a_brms_state_Bmm.cat.rSQ.rds")
...
inf_priors <- c(
   ...
   prior(normal(0, 2.42), class = b, coef="pretty_conditionsparse", dpar = "muangular"),
   prior(normal(0, 2.42), class = b, coef="pretty_conditionsparse", dpar = "muother"),
   prior(normal(0, 2.42), class = b, coef="pretty_conditionsparse", dpar = "mutriangular"),
   prior(normal(0, 2.42), class = b, coef="pretty_conditiongrid", dpar = "muangular"),
   prior(normal(0, 2.42), class = b, coef="pretty_conditiongrid", dpar = "muother"),
   prior(normal(0, 2.42), class = b, coef="pretty_conditiongrid", dpar = "mutriangular")
)
# 
# 
# 
...
Bmm.cat.CrSQ <- brm( state ~ pretty_condition + (1|subject) + (1|q), 
                 ...
                 # backend = "cmdstanr",
                 file ="analysis/SGC4A/models/sgc4a_brms_state_Bmm.cat.CrSQ.rds"
                 )
...
## PLOT predictions
library(ggeffects)
g <- ggpredict(m, terms = c("pretty_condition")) %>% 
...
g
# ggsave(g, filename = "figures/SGC4C_ROTATION_interpretation_predictions.png", width = 6, height =4)
```
