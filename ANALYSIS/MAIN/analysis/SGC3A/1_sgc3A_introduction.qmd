...
#subjects in SGC3A 
#LAB
n_lab <- df_subjects %>% filter( mode == 'lab-synch') %>% nrow()
n_web <- df_subjects %>% filter( mode == 'asynch') %>% nrow()
total = n_lab + n_web

n_ospan <- ospan %>% nrow()

subs_ospan <- ospan %>% dplyr::select(subject)
nrow(subs_ospan)
subs_ospan %>% distinct(subject) %>% nrow()

found <- df_subjects %>% filter(subject %in% subs_ospan$subject) %>% nrow()
```
