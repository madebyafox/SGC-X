...
# (Lab) Hypothesis Testing {#sec-SGC3A-lab-hypotesting}
...
# 
# #EMPTY (random only)
# Bmm.rSQ <- brm( accuracy ~  (1|subject) + (1|q), 
#                  data = df_i, 
#                  family = "bernoulli",
#                  chains = 4, iter = 2000, warmup = 1000,
#                  cores = 4, seed = 1234,
#                  backend = "cmdstanr",
#                  file ="analysis/SGC3A/models/sgc3a_brms_acc_Bmm.rSQ_LAB.rds")
# 
# 
# 
# print("FIXED Condition + Subject & Item random intercepts")
...
# 
# #TEST MODEL
# #Evidence in FAVOR of Condition
# (b <- bayes_factor(Bmm.CrSQ, Bmm.rSQ))
...
# (d <- describe_posterior(m))
(d <- model_parameters(m, exponentiate = TRUE))
...
We fit a (bayesian) multinomial logistic regression model with random intercepts for subjects and questions. A Bayes Factor model comparison (against a random intercepts-only model) indicates extreme evidence for a main effect of CONDITION (BF = 1.95e+17). 
**Consistent with our hypothesis, the impasse condition substantially increases the odds of transitional interpretations.** 

Across the entire task participants in the impasse condition were 13 times more likely to offer an 'unknown' rather than orthogonal response compared with those in the control condition ( $e^{\beta_1} = 12.44, 95 \% CI [6.50, 26.65], pd = 100\%$). Participants in the impasse condition were 23 times more likely to offer an 'angular' rather than orthogonal response compared with those in the control condition ( $e^{\beta_1} = 23.34, 95 \% CI [6.22, 122.37], pd = 100\%$), and 68 times more likely to offer an 'triangular' rather than orthogonal response compared with those in the control condition ( $e^{\beta_1} = 67.60, 95 \% CI [10.28, 612], pd = 100\%$). 
