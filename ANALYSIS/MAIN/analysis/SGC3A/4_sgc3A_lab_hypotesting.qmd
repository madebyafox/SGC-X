...
#IMPORT DATA 
df_subjects <- read_rds('analysis/SGC3A/data/2-scored-data/sgc3a_scored_participants_LAB.rds') %>% 
  filter(mode == "lab-synch") %>% #only get lab-run participants
  mutate(
    task_percent = DV_percent_NABS
  ) %>% droplevels()

df_items <- read_rds('analysis/SGC3A/data/2-scored-data/sgc3a_scored_items_LAB.rds') %>% 
...
(p <- plot_model(m, transform = "exp",
  ...
  labs(title = "MODEL ESTIMATE | Odds Ratio",
       ...
       x = "Condition"))
p
# ggsave(p, filename = "figures/SGC3A_LAB_accuracy_model.png", width = 6, height =4)
...
```
_Based on observed power analysis for the actual effect observed (main effect of condition, on SGC3 study design, with the full model specification), it is indicated to reach 80% power we have ~60 subjects per experimental cell in future studies in which we want to detect an effect similar to that of the impasse condition._
...
df_i = df_items %>% filter(q %nin% c(6,9)) %>% 
  dplyr::select(q,subject,state,pretty_condition)
...
# 
# p <-   
ggbarstats(data = temp, x = state, y = pretty_condition,
               results.subtitle = FALSE,
               ggplot.component = ## modify further with `{ggplot2}` functions
                list(
...
# 
# p <- p + labs(title = "DISTRIBUTION | Interpretation",
#          y = "Proportion of Questions", x = "Condition",
#          subtitle = "")
# 
# p
...
p <- df_i %>% 
  ...
  theme(legend.position = "bottom")+
   labs(title = "DISTRIBUTION | Question Interpretation",
       ...
       subtitle="")
p
# ggsave(p, filename = "figures/SGC3A_LAB_interpretation.png", width = 6, height =4)
# ggsave(p, filename = "figures/SGC3A_LAB_Interpretation.svg", width = 6, height =4)
...
#1 | get draws
# draws <- df_i %>%
#   modelr::data_grid(pretty_condition, subject, q) %>%
#   add_fitted_draws(Bmm.cat.CrSQ,
#   # add_epred_draws(m)
#                    # n = 100,
#                    # dpar = TRUE,
...
p
# ggsave(p, filename = "figures/SGC3A_LAB_interpretation_predictions.png", width = 6, height =4)
```
