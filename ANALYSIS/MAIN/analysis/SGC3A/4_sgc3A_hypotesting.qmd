...
+-----------------------+----------------------------------------------------------------------------------------------------------------------------------------+
| **Alternatives**      | -   Negative Binomial / Zero Inflated Negative Binom for overdispersed count?                                                          |
|                       | -   Beta regression?                                                                                                                   |
...
lab.testabs.lm1 <- lm(DV_percent_test_NABS ~ condition, data = df_lab)
paste("Model")
...
#SCORE predicted by CONDITION
rep.testabs.lm1 <- lm(DV_percent_test_NABS ~ condition, data = df_online)
paste("Model")
...
#SCORE predicted by CONDITION
lab.test_scaled.lm1 <- lm(item_test_SCALED ~ condition, data = df_lab)
paste("Model")
...
#SCORE predicted by CONDITION
rep.test_scaled.lm1 <- lm(item_test_SCALED ~ condition, data = df_online)
paste("Model")
...
The model includes: - A binary logit model to model whether the observation takes a positive count or not. 
- a truncated Poisson or Negative binomial model that only fits positive counts 

This allows us to model: 
(1) Does the student get _any_ questions right?
(2) How many questions does the student get right?
