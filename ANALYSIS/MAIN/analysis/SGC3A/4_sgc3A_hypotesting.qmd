...
_Because nlme, lme4 don't support mixed multinomial, manually fit 2 mixed binomials comparing response states to reference response category [orthogonal].  [alternative: figure out syntax for reformatting data for mclogit [[poorly documented], framed as choice models] _
...
(mc1 <- mblogit(state ~ pretty_condition + subject, weights = Freq,
                        data = df))

#TODO... DO I NEED TO SUMMARIZE?
```
...
plot(mixcat.1)
report(mixcat.1)
check_posterior_predictions(mixcat.1, draws=100)
library(bayesplot)
library(bayestestR)
plot(rope(mixcat.1))
```
