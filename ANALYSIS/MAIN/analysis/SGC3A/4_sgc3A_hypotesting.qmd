...
```{r}
#| label: VIS-SUBJECT-ABS-TEST
 
#VISUALIZE distribution of response accuracy across SUBJECTS

#HISTOGRAM
stats = df_subjects %>% group_by(condition, mode) %>% dplyr::summarise(mean = mean(item_test_NABS))
gf_props(~item_test_NABS, data = df_subjects) %>% 
  ...
  labs(x = "Cumulative Absolute Score",
       ...
       title = "TEST PHASE Total Absolute Score",
...
m1 <- lm(item_test_NABS ~ condition, data = df_subjects %>% filter(mode=="lab-synch"))
paste("Model")
summary(m1)
...
**For in-lab data collection** an OLS linear regression predicting test-phase absolute score by experimental condition explains a statistically significant though small 6% variance in score (F(1,124) = 7.89, p \< 0.01). The estimated beta coefficient ($\beta$ = 1.62, 95% CI \[0.48, 2.76\]) predicts that participants in the impasse condition will on average score 1.6 points (20%) higher than those in the control condition.
...
#SCORE predicted by CONDITION
m1 <- lm(item_test_NABS ~ condition, data = df_subjects %>% filter(mode=="asynch"))
paste("Model")
summary(m1)
paste("Partition Variance")
anova(m1)
...
**For the online replication**, an OLS linear regression model predicting TEST Phase absolute score by condition explains a statistically significant though small 5% of variance in absolute score (F(1,202) = 10.8, p \< 0.01). The beta coefficient for condition indicates that on average, participants in the IMPASSE group scored 1.4 points (18%) higher on the task than those in the control condition (CI\[0.58, 2.30\]).
