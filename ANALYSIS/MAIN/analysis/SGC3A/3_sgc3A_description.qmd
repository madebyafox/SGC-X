...
abs.stats <- rbind(
  ...
  "combined" = df_subjects %>% select(s_ABS) %>% unlist() %>% favstats()
) 
...
Visual inspection of this distribution suggests it is not normal, and perhaps bimodal. We verify this via an excess mass test (Ameijeiras-Alsonso et. al 2018). TODO REFERENCE

```{r}
#| label: CHECK-SUBJ-ABS

multimode::modetest(df_subjects$s_NABS)
n_modes = multimode::nmodes(df_subjects$s_NABS, bw=2)
multimode::locmodes(df_subjects$s_NABS,mod0 =  n_modes, display = TRUE)

```
The excess mass test for multimodality suggests the distribution is multimodal (m=0.1, p < 0.001), with two identifiable modes at 0.26 and 12.3, and antimode at 7. 

```{r}
plot(check_distribution(df_subjects$s_NABS))
