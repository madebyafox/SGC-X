...
abs.stats <- rbind(
  "lab"= df_subjects %>% filter(mode == 'lab-synch') %>% dplyr::select(item_test_NABS) %>% unlist() %>% favstats(),
  "online" = df_subjects %>% filter(mode == "asynch") %>% dplyr::select(item_test_NABS) %>% unlist() %>% favstats(),
...
p <- gghistogram(df_subjects, x = "item_test_NABS", binwidth = 1,
   add = "mean", rug = TRUE,
...
##GGPUBR | HIST+DENSITY SCORE BY CONDITION/MODE
p <- gghistogram(df_online, x = "item_test_NABS",  binwidth = 0.5,
   add = "mean", rug = TRUE,
   fill = "pretty_condition", #, palette = c("#00AFBB", "#E7B800"),
   add_density = TRUE) 
facet(p, facet.by=c("pretty_condition"), nrow = 2) +
  # scale_x_continuous(limits= c(0,8))+
  labs( title = "Distribution of Total Score in TEST Phase",
        subtitle ="",
        x = "Total Absolute Score", y = "number of subjects") +
  theme_minimal() + theme(legend.position = "blank")

##GGPUBR | HIST+DENSITY SCORE BY CONDITION/MODE
p <- gghistogram(df_online, x = "item_test_SCALED",  binwidth = 0.5,
   add = "mean", rug = TRUE,
   fill = "pretty_condition", #, palette = c("#00AFBB", "#E7B800"),
   add_density = TRUE) 
facet(p, facet.by=c("pretty_condition"), nrow = 2) +
...
scaled.stats <- rbind(
  "lab"= df_subjects %>% filter(mode == 'lab-synch') %>% dplyr::select(item_test_SCALED) %>% unlist() %>% favstats(),
  "online" = df_subjects %>% filter(mode == "asynch") %>% dplyr::select(item_test_SCALED) %>% unlist() %>% favstats(),
...
ggplot(data = df_subjects, aes(item_test_SCALED)) + 
  ...
  labs( title = "Empirical Cumulative Density Function — Test Phase Scaled Score",
        x = "Test Phase Absolute Score [-8,8]", 
        y = "Cumulative Probability") + theme_minimal()
