...
#DROP STIMULUS-ENGINE GENERATED SCORES
#these are overriden by scoring file generated scores
df_subjects <- df_subjects %>% select(-absolute_score)
...
To verify that the data collected in the lab and remotely online are comparable, we perform a t-test on group means for each condition. 
```{r}
#| label : MODEL-ABSCORE-BY-MODALITY

paste("Two Sample T-Test for LAB vs. ONLINE control condition")
t.test(data = df_subjects %>% filter(condition == 111), s_ABS ~ mode )

paste("Two Sample T-Test for LAB vs. ONLINE impasse condition")
...
The Cumulative Scaled score `s_SCALED` summarizes the scaled score on the 13 strategy-discriminant questions, for each subject
The range is from -13 (all orthogonal) to 13 (all triangular).  Recall that the s_SCALED score for an item is a numeric representation of the strategy-consitent response, scaled from -1 to +1, where 
"Orthogonal" = -1,
                          "Satisfice" = -1,
                          "Triangular" = 1,
                          "Tversky" = 0.5,
...
ggplot(data = df_items, aes(x = score_niceABS, y = condition)) +
  geom_density_ridges()+ 
  facet_wrap(mode~., labeller = label_both) + 
  labs( x = "Absolute Score for Item") +
  theme_minimal()
...
TODO update this For *in person* collection, item absolute scores (n = `r abs.stats.items["lab",]$n`) range from `r round(abs.stats.items["lab",]$min,2)` to `r round(abs.stats.items["lab",]$max,2)` with a mean score of (M = `r round(abs.stats.items["lab",]$mean,2)`, SD = `r round(abs.stats.items["lab",]$sd,2)`).
