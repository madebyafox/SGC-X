...
library(progressr) #progress bar in for loop
...
df_items$score_SATISFICE = pbmapply(calc_sub_score2, df_items$q, df_items$condition, df_items$response, MoreArgs = list(keyframe = keys_satisfice))
...
for (x in 1:nrow(df_items)) {
  ...
  else if (!is.na(df_items[x,'score_REF'])) #only check if the score is NOT null
    {if( df_items[x,'score_REF'] == 1) {df_items[x,'best'] = "reference"}}
...
df_items$interpretation <- factor(df_items$best, 
                                  levels = c("Orthogonal", "Triangular", "Tversky",
                                             "Satisfice", "reference", "both tri + orth", "blank","frenzy","?"))
