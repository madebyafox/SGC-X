...
for (x in 1:nrow(df_items)) {
  ...
  df_items[x,'s_TRI'] = calc_sub_score(keys_tri, qu, cond, r)
  df_items[x,'s_ORTH'] = calc_sub_score(keys_orth, qu, cond, r)
  df_items[x,'s_TV_max'] = calc_sub_score(keys_tversky_max, qu, cond, r)
  df_items[x,'s_TV_start'] = calc_sub_score(keys_tversky_start, qu, cond, r)
  df_items[x,'s_TV_end'] = calc_sub_score(keys_tversky_end, qu, cond, r)
  df_items[x,'s_TV_duration'] = calc_sub_score(keys_tversky_duration, qu, cond, r)
  
  #calculate special subscores
  df_items[x,'s_REF'] = calc_ref_score(qu, cond, r)
  df_items[x,'s_BOTH'] = calc_both_score(qu, cond, r)
   
  #calculate absolute scores dichotomous
  df_items[x,'score_ABS'] = as.integer(df_items[x,'correct']) 
...
for (x in 1:nrow(df_items)) {
  ...
  if (qu < 6) { #filter by condition
    ...
    #get TVERSKY MAX keys
     tv_max_p = keys_tversky_max %>% filter(Q == qu) %>% filter(condition == cond) %>% select(set_p) %>% pull(set_p) %>% str_split("") %>% unlist()
     tv_max_q = keys_tversky_max %>% filter(Q == qu) %>% filter(condition == cond) %>% select(set_q) %>% pull(set_q) %>% str_split("") %>% unlist()
     tv_max_pn = keys_tversky_max %>% filter(Q == qu) %>% filter(condition == cond) %>% select(n_p)
     tv_max_qn = keys_tversky_max %>% filter(Q == qu) %>% filter(condition == cond) %>% select(n_q)
     
    #get TVERSKY START keys
     tv_start_p = keys_tversky_start %>% filter(Q == qu) %>% filter(condition == cond) %>% select(set_p) %>% pull(set_p) %>% str_split("") %>% unlist()
     tv_start_q = keys_tversky_start %>% filter(Q == qu) %>% filter(condition == cond) %>% select(set_q) %>% pull(set_q) %>% str_split("") %>% unlist()
     tv_start_pn = keys_tversky_start %>% filter(Q == qu) %>% filter(condition == cond) %>% select(n_p)
     tv_start_qn = keys_tversky_start %>% filter(Q == qu) %>% filter(condition == cond) %>% select(n_q)
     
    #get TVERSKY END keys
     tv_end_p = keys_tversky_end %>% filter(Q == qu) %>% filter(condition == cond) %>% select(set_p) %>% pull(set_p) %>% str_split("") %>% unlist()
     tv_end_q = keys_tversky_end %>% filter(Q == qu) %>% filter(condition == cond) %>% select(set_q) %>% pull(set_q) %>% str_split("") %>% unlist()
     tv_end_pn = keys_tversky_end %>% filter(Q == qu) %>% filter(condition == cond) %>% select(n_p)
     tv_end_qn = keys_tversky_end %>% filter(Q == qu) %>% filter(condition == cond) %>% select(n_q)
     
    #get TVERSKY DURATION keys
     tv_duration_p = keys_tversky_duration %>% filter(Q == qu) %>% filter(condition == cond) %>% select(set_p) %>% pull(set_p) %>% str_split("") %>% unlist()
     tv_duration_q = keys_tversky_duration %>% filter(Q == qu) %>% filter(condition == cond) %>% select(set_q) %>% pull(set_q) %>% str_split("") %>% unlist()
     tv_duration_pn = keys_tversky_duration %>% filter(Q == qu) %>% filter(condition == cond) %>% select(n_p)
     tv_duration_qn = keys_tversky_duration %>% filter(Q == qu) %>% filter(condition == cond) %>% select(n_q)   
     
    #get REFERENCE POINT keys
     #1. GET reference point from ALSO column in raw keys
     ref_p = keys_raw %>% filter(Q == qu) %>% filter(condition == cond) %>% select(ALSO) %>% pull(ALSO) %>% str_split("") %>% unlist()
     #2. get all possible options [doesn't matter which key, they're all the same]
     ref_n =keys_raw %>% filter(Q == qu) %>% filter(condition == cond) %>% select(OPTIONS) %>% pull(OPTIONS) %>% str_split("") %>% unlist() 
     #3. calc answers that aren't the reference point
     ref_q = setdiff(ref_n,ref_p)
     #4. cal number of ps and qs 
     ref_pn = length(ref_p)
     ...
     #Manually derive key for BOTH interpretation
     #1. calc answer that is both tri and orth and only these --> union of tri_p and orth_p
     both_p = union(tri_p, orth_p) #the selection of tri and p
     #2. get all possible options [doesn't matter which key, they're all the same]
     both_n =keys_tri %>% filter(Q == qu) %>% filter(condition == cond) %>% select(set_n) %>% pull(set_n) %>% str_split("") %>% unlist() 
     #3. calc answers that should't be selected as diffrence between N [same for all keys] and both_p
     both_q = setdiff(both_n,both_p)
     both_pn = length(both_p)
     both_qn = length(both_q)
        
     
    
  } else { #don't filter on condition; all Q>6 are the same
    ...
    orth_qn = keys_orth %>% filter(Q == qu) %>% select(n_q)
     ...
     #get TVERSKY MAX keys
     tv_max_p = keys_tversky_max %>% filter(Q == qu) %>% select(set_p) %>% pull(set_p) %>% str_split("") %>% unlist()
     tv_max_q = keys_tversky_max %>% filter(Q == qu) %>% select(set_q) %>% pull(set_q) %>% str_split("") %>% unlist()
     tv_max_pn = keys_tversky_max %>% filter(Q == qu) %>% select(n_p)
     tv_max_qn = keys_tversky_max %>% filter(Q == qu) %>% select(n_q)
     
      #get TVERSKY START keys
     tv_start_p = keys_tversky_start %>% filter(Q == qu) %>% select(set_p) %>% pull(set_p) %>% str_split("") %>% unlist()
     tv_start_q = keys_tversky_start %>% filter(Q == qu) %>% select(set_q) %>% pull(set_q) %>% str_split("") %>% unlist()
     tv_start_pn = keys_tversky_start %>% filter(Q == qu) %>% select(n_p)
     tv_start_qn = keys_tversky_start %>% filter(Q == qu) %>% select(n_q)
     
     #get TVERSKY END keys
     tv_end_p = keys_tversky_end %>% filter(Q == qu) %>% select(set_p) %>% pull(set_p) %>% str_split("") %>% unlist()
     tv_end_q = keys_tversky_end %>% filter(Q == qu) %>% select(set_q) %>% pull(set_q) %>% str_split("") %>% unlist()
     tv_end_pn = keys_tversky_end %>% filter(Q == qu) %>% select(n_p)
     tv_end_qn = keys_tversky_end %>% filter(Q == qu) %>% select(n_q) 
     
     #get TVERSKY DURATION keys
     tv_duration_p = keys_tversky_duration %>% filter(Q == qu) %>% select(set_p) %>% pull(set_p) %>% str_split("") %>% unlist()
     tv_duration_q = keys_tversky_duration %>% filter(Q == qu) %>% select(set_q) %>% pull(set_q) %>% str_split("") %>% unlist()
     tv_duration_pn = keys_tversky_duration %>% filter(Q == qu) %>% select(n_p)
     tv_duration_qn = keys_tversky_duration %>% filter(Q == qu) %>% select(n_q)  
     
     #get REFERENCE POINT keys
     #1. GET reference point from ALSO column in raw keys
     ref_p = keys_raw %>% filter(Q == qu) %>% select(ALSO) %>% pull(ALSO) %>% str_split("") %>% unlist()
     #2. get all possible options [doesn't matter which key, they're all the same]
     ref_n =keys_raw %>% filter(Q == qu) %>% select(OPTIONS) %>% pull(OPTIONS) %>% str_split("") %>% unlist() 
     #3. calc answers that aren't the reference point
     ref_q = setdiff(ref_n,ref_p)
     #4. cal number of ps and qs 
     ref_pn = length(ref_p)
     ...
     both_p = union(tri_p, orth_p) #the selection of tri and p
     #2. get all possible options [doesn't matter which key, they're all the same]
     both_n =keys_tri %>% filter(Q == qu) %>% select(set_n) %>% pull(set_n) %>% str_split("") %>% unlist() 
     #3. calc answers that should't be selected as diffrence between N [same for all keys] and both_p
     both_q = both_n - both_p
     both_pn = length(both_p)
     both_qn = length(both_q)
  }
  ...
  tv_max_ps = length(intersect(r,tv_max_p))
  tv_max_qs = length(intersect(r,tv_max_q))
  df_items[x,'tv_max_ps'] = tv_max_ps
  df_items[x,'tv_max_qs'] = tv_max_qs
  
  tv_start_ps = length(intersect(r,tv_start_p))
  tv_start_qs = length(intersect(r,tv_start_q))
  df_items[x,'tv_start_ps'] = tv_start_ps
  df_items[x,'tv_start_qs'] = tv_start_qs
  
  tv_end_ps = length(intersect(r,tv_end_p))
  tv_end_qs = length(intersect(r,tv_end_q))
  df_items[x,'tv_end_ps'] = tv_end_ps
  df_items[x,'tv_end_qs'] = tv_end_qs

  tv_duration_ps = length(intersect(r,tv_duration_p))
  tv_duration_qs = length(intersect(r,tv_duration_q))
  df_items[x,'tv_duration_ps'] = tv_duration_ps
  df_items[x,'tv_duration_qs'] = tv_duration_qs
  
  ref_ps = length(intersect(r,ref_p))
  ref_qs = length(intersect(r,ref_q))
  df_items[x,'both_ps'] = ref_ps
  df_items[x,'both_qs'] = ref_qs
  
  both_ps = length(intersect(r,both_p))
  both_qs = length(intersect(r,both_q))
  df_items[x,'both_ps'] = both_ps
  df_items[x,'both_qs'] = both_qs
  ...
  df_items[x,'score_BOTH'] = f_partialP(both_ps,both_pn,both_qs,both_qn)
  df_items[x,'score_REF'] = f_partialP(ref_ps,ref_pn,ref_qs,ref_qn)
  df_items[x,'score_TV_max'] = f_partialP(tv_max_ps,tv_max_pn,tv_max_qs,tv_max_qn)
  df_items[x,'score_TV_start'] = f_partialP(tv_start_ps,tv_start_pn,tv_start_qs,tv_start_qn)
  df_items[x,'score_TV_end'] = f_partialP(tv_end_ps,tv_end_pn,tv_end_qs,tv_end_qn)
  df_items[x,'score_TV_duration'] = f_partialP(tv_duration_ps,tv_duration_pn,tv_duration_qs,tv_duration_qn)
  
  
  #calculate permissive dichotomous
  #for each item there is a single correct response 
  #BUT there are also sometimes one or more technically-incorrect options that might have been selected
  #for which we don't want to penalize the subject. For example, selecting the reference point of the 
  #question IN ADDITION to the correct option.  (ie which start at the same time as X? if they select 
  #the correct answer Y and also select the reference point X)
  #These 'allowable' items are already accounted for in the triangular score logic, because the options
  #that are 'allowed' are already excluded from the set_q in the keys_tri dataframe
  df_items[x,'score_niceABS'] = (df_items[x,'score_TRI'] == 1)
  df_items[x,'score_niceABS'] = as.integer(df_items[x,'score_niceABS'])
  
  #ALSO recast absolute score (strict ABS, not being nice and allowing the reference and visual error points)
  df_items[x,'score_ABS'] = as.integer(df_items[x,'correct'])
}

#replace NaNs with 0
#this occurs when the set of P answers is empty (ie. some of the tversky sets)
# df_items$score_TV_duration[is.nan(df_items$score_TV_duration)] <- 0

#cleanup
rm(qu, cond, r,x)
rm(orth_pn, orth_p, orth_qn, orth_q, orth_ps, orth_qs,  
   tri_pn, tri_p, tri_qn, tri_q, tri_ps, tri_qs, 
   tv_duration_pn, tv_duration_p, tv_duration_qn, tv_duration_q, tv_duration_ps, tv_duration_qs,
   tv_end_pn, tv_end_p, tv_end_qn, tv_end_q, tv_end_ps, tv_end_qs,
   tv_start_pn, tv_start_p, tv_start_qn, tv_start_q, tv_start_ps, tv_start_qs,
   tv_max_pn, tv_max_p, tv_max_qn, tv_max_q, tv_max_ps, tv_max_qs,
   both_pn, both_p, both_qn, both_q, both_ps, both_qs,
   ref_pn, ref_p, ref_qn, ref_q, ref_ps, ref_qs)
#TODO rm all ps and qs from df_items
```
...
threshold_variance = 0.2 #set required variance in subscores to be discriminant
threshold_frenzy = 7
for (x in 1:nrow(df_items)) {
  ...
  df_items[x,'score_TVERSKY'] = max(t.long$value)
  ...
  s.long = gather(t,score, value, 1:3)
  
  #calculate and store variance in subscores
  v = var(s.long$value)  
  df_items[x,'var'] = v
