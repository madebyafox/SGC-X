...
subjects_q1 <- df_items %>% filter(q == 1) %>% mutate(
  ...
  item_q1_rt = rt_s,
) %>% dplyr::select(subject, item_q1_NABS, item_q1_interpretation,item_q1_rt) %>% arrange(subject)
...
subjects_q5 <- df_items %>% filter(q == 5) %>% mutate(
  ...
  item_q5_rt = rt_s,
) %>% dplyr::select(subject, item_q5_NABS, item_q5_interpretation,item_q5_rt) %>% arrange(subject) 
...
```{r}
#| label: SUMMARIZE-CUMULATIVE PROGRESS


#filter for valid items
x <- df_items %>% filter(q %nin% c(6,9)) %>% select(subject,condition, q,score_niceABS)
...
wide <- wide %>% select(subject,condition,c1,c2,c3,c4,c5,c6, c7,c8,c9, c10,c11,c12,c13)

#lengthen 
long <- wide %>% pivot_longer(cols= c1:c13, names_to = "question", names_pattern = "c(.*)", values_to = "score")
long$question <- as.integer(long$question)


subs <- df_subjects$subject


# long_perfect <- perfect %>% pivot_longer(!subject, names_to = "step", values_to = "score")
df <- long %>% filter(subject %in% (subs %>% head(100)))
ggplot(data = long, aes(x = question, y = score, group = subject, alpha = 0.01, color = condition)) + 
