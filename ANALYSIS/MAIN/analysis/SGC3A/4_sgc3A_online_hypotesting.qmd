...
# Hypothesis Testing {#sec-SGC3A-online-hypotesting}
...
df_subjects <- read_rds('analysis/SGC3A/data/2-scored-data/sgc3a_scored_participants_ospan.rds') 
...
*The Condition predictor decreases AIC, and the Likelihood Ratio Test indicates a better fit than the empty model.*


##### Fit OSPAN Model

*NEXT, we fit a logistic regression with OSPAN as predictor, and compare its fit to the condition model.*
...
#: 3 TEST SUPERIOR FIT
paste("AIC wth OSPAN is lower than CONDITION model?", m.C$aic > m.O$aic)
...
#: 4 OSPAN + CONDITION INTERACTION model
...
#: 3 TEST SUPERIOR FIT
paste("AIC wth OSPAN is lower than CONDITION only model?", m.CO$aic > m.C.O$aic)
test_lrt(m.CO,m.C.O) #same as anova(m0, m1, test = "Chi")
paste("Likelihood Ratio test is significant? p = ",(test_lrt(m.CO,m.C.O))$p[2])
```
