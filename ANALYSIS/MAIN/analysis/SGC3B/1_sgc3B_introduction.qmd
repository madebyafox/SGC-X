---
subtitle: 'Study SGC3B | 1 Introduction'
---

\newpage
**TODO UPDATE ALL**

# Introduction {#sec-SGC3B-introduction}
...
|                                  | Demo: [111](https://limitless-plains-85018.herokuapp.com/?study=SGC3B&condition=111&session=WEB-DEMO) |
+----------------------------------+-------------------------------------------------------------------------------------------------------+
| ![](/analysis/utils/img/121.png) | **Impasse-Condition**\                                                                                |
|                                  | Demo: [121](https://limitless-plains-85018.herokuapp.com/?study=SGC3B&condition=121&session=WEB-DEMO) |
+----------------------------------+-------------------------------------------------------------------------------------------------------+
...
#IMPORT DATA 
df_subjects <- read_rds('analysis/SGC3B/data/1-study-level/sgc3b_participants.rds')
...
cols = c("NONE-NONE","NONE-Impasse","IMG-NONE","IMG-Impasse","IXV-NONE","IXV-Impasse","Total for Period")
...
![Sample Question (Q=1) graphs for each condition](static/stimuli/3B_conditions.png){#fig-conditions fig-alt="a triangle graph at left shows the correct (diagonal) path from the x-axis to the correct answer(datapoint f) in green, and an incorrect orthogonal projection from the x-axis to incorrect datapoint A.  At right, the same graph is depicted, but datapoint A no longer intersects the orthogonal projection from the xaxis."}
...
### Data Preparation {#sec-SGC3B-harmonize}
...
+=================================================================================================================+=====================+
| spring17_clean_data.Rmd <br> spring18_clean_data.Rmd <br> fall21_clean_data.Rmd <br> winter2022_clean_sgc3b.Rmd | 2_sgc3B_scoring.qmd |
+-----------------------------------------------------------------------------------------------------------------+---------------------+
...
| Summer 2022   | asynchronous, online, SONA             |
...
map_relations <- df_items_summer21 %>% group_by(q) %>% dplyr::select(q,relation) %>% unique()

#SAVE METADATA FROM SUMMER, but no rows 
df_items <- df_items_summer21 %>% filter(condition=='X') %>% dplyr::select(
  subject,condition,term,mode,
