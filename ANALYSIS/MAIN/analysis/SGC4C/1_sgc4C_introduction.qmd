...
# WIP TODO Introduction {#sec-SGC4C-introduction}
...
#IMPORT DATA 
df_subjects <- read_rds('analysis/SGC4C/data/0-session-level/sgc4c_participants.rds')
...
#import control participants
df_control <- read_rds("analysis/SGC3A/data/1-study-level/sgc3a_participants.rds") %>% filter(condition==111) %>% dplyr::select(-pretty_mode) %>% droplevels() %>% mutate(
  study = "SGC3A"
)
#import triangular particiapants
df_tri <- read_rds("analysis/SGC4A/data/1-study-level/sgc4a_participants.rds") %>% filter(condition==113) %>% dplyr::select(-pretty_mode) %>% droplevels() %>% mutate(
  study = "SGC4D"
)
...
#import rotation participants
df_rotated <- read_rds("analysis/SGC4C/data/2-scored-data/sgc4c_scored_participants.rds") 
rotated_items <- read_rds("analysis/SGC4C/data/2-scored-data/sgc4c_scored_items.rds") %>% mutate(override="") %>% dplyr::select(-answer)

#import control participants
df_control <- read_rds("analysis/SGC3A/data/2-scored-data/sgc3a_scored_participants_REP.rds") %>% filter(condition==111) %>% dplyr::select(-pretty_mode) %>% droplevels() %>% mutate(
  study = "SGC3A"
)
control_items <- read_rds("analysis/SGC3A/data/2-scored-data/sgc3a_scored_items_REP.rds") %>% filter(condition==111) %>% dplyr::select(-pretty_mode) %>% droplevels() %>% mutate(
  study = "SGC3A"
) %>% dplyr::select(-IMPASSE)

#import triangular particiapants
df_tri <- read_rds("analysis/SGC4A/data/2-scored-data/sgc4a_scored_participants.rds") %>% filter(condition==113) %>% dplyr::select(-pretty_mode) %>% droplevels() %>% mutate(
  study = "SGC4D"
)
tri_items <- read_rds("analysis/SGC4A/data/2-scored-data/sgc4a_scored_items.rds") %>% filter(condition==113) %>% dplyr::select(-pretty_mode) %>% droplevels() %>% mutate(
  study = "SGC4D"
) %>% dplyr::select(-IMPASSE)

# janitor::compare_df_cols(rotated_items,control_items, tri_items)

df_all <- rbind(df_rotated, df_control, df_tri) %>% mutate(
  ROTATION = recode_factor(condition, 
                                   "111"="none", "113"="none",
                           "111112" = "45deg", "113112" = "45deg"),
  SHAPE = recode_factor(condition, 
                                   "111"="orth", "113"="tri",
                           "111112" = "orth", "113112" = "tri"),
)
df_items <- rbind(rotated_items, control_items, tri_items)  %>% mutate(
  ROTATION = recode_factor(condition, 
                                   "111"="none", "113"="none",
                           "111112" = "45deg", "113112" = "45deg"),
  SHAPE = recode_factor(condition, 
...
library(ggstatsplot)
grouped_ggbetweenstats( data = df_all, y = s_NABS, grouping.var = ROTATION, x = SHAPE)
```
