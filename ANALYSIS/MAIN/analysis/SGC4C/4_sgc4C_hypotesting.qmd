...
# SGC4C Hypothesis Testing {#sec-SGC4C-hypotesting}
...
In SGC4C-OSPAN we set out to answer the following question: Does posing a mental impasse improve performance on the interval graph comprehension task? Does WORKING MEMORY CAPACITY (as measured by the OSPAN task) explain performance on the graph comprehension task?
...
df_subjects <- read_rds('analysis/SGC4C/data/2-scored-data/sgc4c_scored_participants.rds') 
...
title = "Descriptive Statistics of Response Accuracy (Total % Correct) BY FACTORIAL CONDITION"
tbl2 <- mosaic::favstats(task_percent ~ SHAPE+ROTATION, data = df_s) 
tbl2 %>% kbl (caption = title) %>% kable_classic()
...
## 3 | ADD FIXED EFFECT ROTATION

print("FIXED Rotation + Subject & Item random intercepts")
mm.RrSQ <- glmer(accuracy ~ ROTATION + (1|subject) + (1|q) ,
                data = df_i, family = "binomial")
#summary(mm.RrSQ)

paste("AIC decreases w/ new model", AIC(logLik(mm.rSQ)) > AIC(logLik(mm.RrSQ)) )
test_lrt(mm.rSQ,mm.RrSQ) #same as anova(m0, m1, test = "Chi")
paste("Likelihood Ratio test is significant? p = ",(test_lrt(mm.rSQ,mm.RrSQ))$p[2])

## 4 | ADD FIXED EFFECT SHAPE
...
paste("AIC decreases w/ new model", AIC(logLik(mm.RrSQ)) > AIC(logLik(mm.R.S.rSQ)) )
test_lrt(mm.RrSQ,mm.R.S.rSQ) #same as anova(m0, m1, test = "Chi")
paste("Likelihood Ratio test is significant? p = ",(test_lrt(mm.RrSQ,mm.R.S.rSQ))$p[2])


## 3 | ADD INTERACTION TER,
...
# best model
# m <- mm.RSrSQ or mm.R.S.rSQ
# m %>% write_rds(file = "analysis/SGC4C/models/sgc4c_glmer_acc_mm.COrSQ_OSPAN.rds")
...
(b <- bayesfactor(Bmm.cat.R.SrSQ, Bmm.cat.RSrSQ))
...
# best model
# m <- Bmm.cat.RSrSQ or Bmm.cat.R.SrSQ
...
df <- df_items %>% filter(q==1) %>% dplyr::select(accuracy, pretty_condition, ROTATION, SHAPE)
...
table(df$accuracy, df$ROTATION, df$SHAPE) %>% addmargins(2) %>% #display sum for row
      addmargins(1) #sanity check sum of columns
```
...
#CHI SQUARE ON ACCURACY X ROTATION in TRI
df_tri <- df %>% filter(SHAPE == "TRI")
table(df_tri$ROTATION, df_low$accuracy)
chisq.test( x = df_tri$ROTATION, y = df_tri$accuracy, correct = TRUE)

#CHI SQUARE ON ACCURACY X ROTATION in ORTH
df_orth <- df %>% filter(ospan_split == "high-memory")
table(df_orth$ROTATION, df_high$accuracy)
chisq.test( x = df_orth$ROTATION, y = df_orth$accuracy,correct = TRUE)
...
grouped_ggbarstats( data = df, x = accuracy, y = ROTATION, 
                    grouping.var = SHAPE,
                    type = "nonparametric")
...
#: 2 SIMPLE CONDITION model
m.C <- glm( accuracy ~ pretty_condition, data = df, family = "binomial")


#: 3 MAIN EFFECTS FACTORIAL MODEl 
m.R.S = glm(accuracy ~ ROTATION + SHAPE , data = df, family = "binomial")


#: 4 INTERACTION MODEL
m.RS <- glm( accuracy ~ ROTATION*SHAPE data = df, family = "binomial")


#: TEST FIT FIT
paste("SIMPLE CONDIDTION better than empty?")
test_lrt(m.C,m.0) #same as anova(m0, m1, test = "Chi")

paste("MAIN EFFECTS better than empty?")
test_lrt(m.R.S,m.0) 

paste("INTERACTION better than MAIN EFFECTS?")
test_lrt(m.RS,m.R.S) 
...
#              notes = notes,
#              output = "analysis/SGC4C/models/tables/GLMER_OverallAccuracy_lab.tex")
# #              # coef_omit = "Intercept",
...
## LEFT OFF HERE Q1 INTERPRETATION STATE
...
df <- df_items %>% filter(q==1) %>% 
  dplyr::select(pretty_condition, state, ROTATION, SHAPE)
...
(t <- table(df$state, df$ROTATION, df$SHAPE) %>% addmargins(2) %>% #display sum for row
      addmargins(1)) #sanity check sum of columns

```

#### TODO LEFT OFF HEREMULTINOMIAL REGRESSION
