...
df_i = df_items %>% filter(q %nin% c(6,9)) %>% 
  ...
  mutate(
    ROTATION = recode_factor(ROTATION,
                             "0"="0deg",
                             "45"="45deg",
                             "90"="90deg")
  )
...
x <-  glmer(accuracy ~ SHAPE + ROTATION + (1|subject) + (1|q) ,
                data = df_i, family = "binomial")
#summary(mm.R.S.rSQ)
...
m <- read_rds(file = "analysis/SGC4C/models/sgc4c_glmer_acc_mm.R.S.rSQ.rds")
...
p <- ggpredict(x, terms = c("ROTATION","SHAPE")) %>% 
...
p
# ggsave(p, filename = "figures/SGC4C_ROTATION_accuracy_prediction.png", width = 6, height =4)
```
...
g
# ggsave(g, filename = "figures/SGC4C_ROTATION_interpretation.png", width = 6, height =4)
```
...
# 
# p <-   
grouped_ggbarstats(data = temp, x = state, y = ROTATION,
                          grouping.var = SHAPE,
               results.subtitle = FALSE,
               ggplot.component = ## modify further with `{ggplot2}` functions
                list(
...
## PLOT INTERACTION
library(ggeffects)
g <- ggpredict(m, terms = c("ROTATION","SHAPE")) %>% 
