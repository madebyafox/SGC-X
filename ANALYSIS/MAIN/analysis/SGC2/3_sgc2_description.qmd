...
Data was initially collected (in person, SONA groups in computer lab) in Winter and Spring 2017. 
...
Subject level scores summarize the the response accuracy by a particular participant across all blocks of the two graph comprehension tasks. 
...
ggplot(long_scores, aes(x = graph, y = score,
  ...
  geom_point(
    ...
    title = "Distribution of Score by Graph ",
...
ggplot(data = df_absolute_progress, aes(x = question, y = score, group = subject, alpha = 0.01, color = condition)) + 
...
ggplot(data = df_scaled_progress, aes(x = question, y = score, group = subject, alpha = 0.01, color = condition)) + 
 geom_line(position=position_jitter(w=0.15, h=0.15), size=0.1) +
 facet_wrap(~condition) + 
