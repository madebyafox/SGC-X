...
```{r}
table(in_sgc4a$condition)

df_s <- in_sgc4a
df_i <- in_sgc4a_items %>% filter(q %nin% c(6,9)) %>% mutate(
  q = as.factor(q),
...
print("FIXED Condition + Subject ONLY random intercepts")
mm.CrS <- glmer(accuracy ~ pretty_condition + (1|subject),
                data = df_i, family = "binomial")
summary(mm.CrS)

compare_performance(mm.CrSQ, mm.CrS)
...
# #save table for latex in dissertation
