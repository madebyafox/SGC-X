...
# setwd("~/Sites/RESEARCH/SGC—Scaffolding Graph Comprehension/SGCx/ANALYSIS/SGC3_B")
```
...
# df_fall <- df_fall %>% filter(condition != 111) %>% filter(condition != 121)
# df_spring <- df_spring 
# df_subjects <- rbind(df_fall, df_spring)
# df_subjects <- df_spring
...
gf_dhistogram(~tri_min, fill = ~condition, data = df_subjects) %>% 
  gf_facet_grid(condition~.)
```
The average total response latency (entire 15 question block) for the control condition was slightly higher (M = 9.54s, SD = 3.09) than the most-scaffolded (explicit: interactive, implicit:impasse) condition (M = 7.11, 1.81s)
...
```
The difference in average response time IS statistically significant F(5,284) = 8.57 , p < 0.01, with a linear model predicting response time from condition explaining only 13% of variance in response time. 
