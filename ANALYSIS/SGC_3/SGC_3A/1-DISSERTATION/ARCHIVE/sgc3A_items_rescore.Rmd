...
library(stringr) #string comparisons
library(tidyr) #pivot
...
df_nondiscriminant <- df_items %>% filter(q %in% c(6,9,13)) 
df_items <- df_items %>% filter (!q %in% c(6,9,13,16))
...
test_111 <- df_items %>% dplyr::select(condition,q,answer) %>% filter(condition==111) %>%  group_by(q,condition,answer) %>% summarize(count=n())
test_121 <- df_items %>% dplyr::select(condition,q,answer) %>% filter(condition==121) %>% group_by(q,condition,answer) %>% summarize(count=n())
```
...
df_items <- df_items %>% dplyr::select(subject, condition, q, answer, rs_tri, rs_ortho)
...
#4a | Intersections of response with each response mode
df_control <- df_control %>% rowwise() %>% mutate(
    a = str_split(answer,""), 
<<<<<<< HEAD
    tri = str_split(TRI_CORRECT,""),
    orth = str_split(ORTH_CORRECT,""),
    tri_sect = list(intersect(a,tri)), #matches to correct components
    orth_sect = list(intersect(a,orth)), #matches to correct components
    clicky = nchar(answer) - nchar(TRI_CORRECT), #difference between actual & correct num data points
...
=======
    ...
    TRI = peritem_tri * length(intersect)
  ) %>% dplyr::select(-a,-b,-intersect) 
>>>>>>> 
  b2da5d479e97c9bf77275751728eae29a11524a1
...
# df_control$trisect <- paste(df_control$TRIANSWER, df_control$answer)
  ...
  # else {
  # let instrategy = _.union(tri,orth,also); 
