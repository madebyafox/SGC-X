---
title: "Winter 2022 SGC 4C Data Cleaning"
author: "Amy Rae Fox"
date: "07/22/2022"
...
#IMPORT DATA
df_participants <- fromJSON("input/WIP_su22_sgc4c_final_participants.json")
df_items <- fromJSON('input/WIP_su22_sgc4c_final_items.json')
...
conditions <-c(113112,111112) #3 conditions
...
df_participants <- df_participants %>%  
  mutate( #create factors and remove extraneous ""
    ...
    pretty_condition = recode_factor(condition, "113112" = "TRI-rotate-45", "111112" =  "ORTH-rotate-45"),
    study = factor(study),
...
_One participant record was excluded on account of session (ie. app testing or pilot session)._
...
_Four participants were excluded for exceeding the maximum allowed number of browser interaction violations._
...
_No participants are excluded for low (self-rated) effort._
...
_Three participants are excluded for failing the attention check question._
...
library(mosaic)
t.test(df_participants$absolute_score ~ df_participants$condition)
w <- wilcox.test(df_participants$absolute_score ~ df_participants$condition)
w
report(w)
...
library(performance)
library(report)
m <- lm(absolute_score ~ condition, data = df_participants)


library(effectsize)
library(effsize)
cohen.d(absolute_score ~ condition, data = df_participants)
...
q1 <- df_items %>% filter(q==1)
m <- glm(as.factor(correct) ~ pretty_condition, data = q1, family = "binomial")
summary(m)
exp(coef(m))
exp(confint(m))
report(m)
...
library(lme4)
m <- glmer(correct ~ pretty_condition + (1 | subject), data = df_items, family = "binomial")
summary(m)
report(m)


library(ggformula)
gf_boxplot(absolute_score ~ pretty_condition, widht = 0.10, data = df_participants) %>% 
  gf_jitter(alpha=0.5, width = 0.1)
```