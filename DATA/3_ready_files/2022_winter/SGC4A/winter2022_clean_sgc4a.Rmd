...
session_dir = "/Users/amyfox/Sites/RESEARCH/SGC—Scaffolding Graph Comprehension/SGC-X/DATA/3_ready_files/2022_winter"
knitr::opts_knit$set(session_dir)
...
df_participants <- df_participants %>%  
  mutate( #create factors and remove extraneous ""
    ...
    pretty_condition = recode_factor(condition, "11111" = "Orth-Full", "114" =  "Orth-Sparse", "115" = "Orth-Grid", "113" = "Tri-Sparse"),
    study = factor(study),
    condition = factor(condition),
...
rm(df_participants_control,df_participants_experimental, df_participants_keep, df_participants_split, df_items_keep, df_items_split)
```
...
write.csv(ex_participants,"output/excluded_participants_sgc4A.csv", row.names = FALSE)
write.csv(ex_items,"output/excluded_items_sgc4A.csv", row.names = FALSE)
...
write.csv(df_participants,"output/sgc4a_participants.csv", row.names = FALSE)
#save item file
write.csv(df_items,"output/sgc4a_items.csv", row.names = FALSE)

#export R DATA STRUCTURES (include codebook metadata)
rio::export(df_participants, "output/sgc4a_participants.rds") # to R data structure file
rio::export(df_items, "output/sgc4a_items.rds") # to R data structure file
