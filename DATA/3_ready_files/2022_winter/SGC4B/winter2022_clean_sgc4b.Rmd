---
title: "Winter 2022 SGC 4B Data Cleaning"
author: "Amy Rae Fox"
date: "04/07/2022"
...
*note*: The SGC4B data files do not contain the 'explanation' column (the subject's typed explanation to the free response question asking them to explain a particular question.) I'm not certain why this field is not in the dataset, but I've traced it back to the DB records and see it is not in the jsPsych trial level data. Most likely these data were collected at the start of the data collection period, and this question was added to the study afterward. This is not a critical problem, because the explanation question is an exploratory analysis and not used for hypothesis testing. 
...
#TODO | verify that 4bCONTROL has been copied to input folder; out of 4A output folder
control_participants<- read.csv("input/winter22_sgc4b_CONTROL_participants.csv") %>% 
select(-explanation, -discriminant_score, -tri_score, -orth_score, -other_score, -blank_score)  %>%
mutate(
  condition= as.character(condition),
  subject = as.character(subject))
...
control_items<- read.csv("input/winter22_sgc4b_CONTROL_items.csv") %>% 
  select( -discriminant, -tri_score, -orth_score, -other_score, -blank_score)
...
_Thirty seven participants were excluded for exceeding the maximum allowed number of browser interaction violations._
...
_Thirty-six participants are excluded for low (self-rated) effort._
...
_Twenty three participants are excluded for failing the attention check question._
...
gf_histogram( ~absolute_score ,data = df_participants) + 
  labs(title = "SGC4B Distribution of Absolute Score")

gf_histogram( ~absolute_score ,data = df_participants) %>% 
  gf_facet_wrap(~pretty_condition) +
  labs(title = "SGC4B Distribution of Absolute Score (by Condition)")

gf_props(~correct, data = df_items) + 
  labs(title = "SGC4B Distribution of Item Absolute Score")

gf_props(~correct, data = df_items) %>% 
  gf_facet_wrap(~pretty_condition) + 
  labs(title = "SGC4B Distribution of Item Absolute Score (by Condition)")
...
gf_histogram( ~absolute_score ,data = df_participants) %>% 
  gf_facet_wrap(~pretty_condition) +
  labs(title = "SGC4B Distribution of Total Study Time")

gf_histogram(~rt_s, data = df_items) + 
  labs(title = "SGC4B Distribution of Item Response Time")
...
#CSV files
write.csv(df_participants,"output/sgc4b_participants.csv", row.names = FALSE)
write.csv(df_items,"output/sgc4b_items.csv", row.names = FALSE)

#export R DATA STRUCTURES (include codebook metadata)
rio::export(df_participants, "output/sgc4b_participants.rds") # to R data structure file
rio::export(df_items, "output/sgc4b_items.rds") # to R data structure file
