...
_20 participants were excluded for exceeding the maximum allowed number of browser interaction violations._
...
_Thirty-nine participants are excluded for low (self-rated) effort._
...
df_participants %>% group_by(attn_check, condition) %>% 
  dplyr::summarize(n=n())
```
...
exclude_attn <- df_participants %>%
          filter(attn_check == FALSE & condition == 1112) %>%
