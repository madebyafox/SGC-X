...
## —————————————————————————————————————
## Read in .csv containing OSPAN data (explorted from Qualtrics)
## SCORE OSPAN data 
## output file csv file ready to be imported into R for joining and analysis with SGC data
## —————————————————————————————————————

library(tidyverse)

# read in qualtrics export
df_ospan <- read_csv("OSPAN_results.csv")

#POP FIRST TWO ROWS which contain qualtrics info
n_rows <- df_ospan %>% nrow
...
#CREATE SUBJECT LEVEL DF 
df_subjects <- df_ospan %>% filter(
  str_length(SUBJECTID) == 5 #filters incorrectly entered subids 
) %>% select(
  `StartDate` : `SUBJECTID`, `QUALTRICSID`
)

# CREATE ITEMS 
df_items <- df_ospan %>% filter(
  str_length(SUBJECTID) == 5 #filters incorrectly entered subids 
) %>% select(
  `SUBJECTID` : `6BRe_ucsbsk`
)


#Calculate accuracy on the math questions 
df_math <- df_items  %>% #Clean dataframe
  select(SUBJECTID, matches('^[0-9].A.*|^4aQ8_F$')) %>% #Get subject ID and match all of the math problem columns
  ...
  group_by(SUBJECTID) %>% #Keep these columns and get scores for each participant
  summarise(acc = mean(score),
            total = sum(score),
            n = n()) %>% #Average their math score within participant (0-1)
  ungroup()

#ANSWER KEY FOR OSPAN ORDER
...
#Calculate order correct score 
df_order <- df_items %>%
  select("SUBJECTID", matches('^[0-9].Re_[a-z]+_[0-9]_x|^[0-9].Re_[a-z]+_[0-9]_y')) %>%
  ...
  select(SUBJECTID,orderscore = score) %>%#These are the only columns needed to join with the math score
  group_by(SUBJECTID) %>% #Get score by subject ID
  summarise(orderscore = sum(orderscore)) #Summarise the orderscore as a raw sum out of 30


#JOIN math and order scores
df_scored <- left_join(df_math,df_order) %>% #Join the OSPAN ordered score and the math accuracy
  mutate(
    order_num = orderscore, 
    math_acc = acc,
    order_acc = order_num/30,
    weighted_order = math_acc * order_num
  ) %>% 
  select(SUBJECTID, order_num, math_acc, order_acc, weighted ) 

#OUTPUT DATAFRAME
write_csv(df_scored, "../study_files/fall21_scored_ospan.csv")
