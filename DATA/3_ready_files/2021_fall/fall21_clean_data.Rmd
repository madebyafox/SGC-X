...
### Sessions
In FALL21, the session code was used to capture the participants UCSD PID (for exclusion in future studies) The (string) session code is entered by the participant based on instructions given by the experimenter, and documents the data-collection session (eg. in-person at a particular time). This code is also used by the experimenter to differentiate test or expert data collection runs.  
```{r SESSIONS}
...
#manually recode sessions in blocks
df_blocks$condition <- dplyr::recode(df_blocks$condition,
                                     '121\n121'='121',
                                  '221\n221'='221')
