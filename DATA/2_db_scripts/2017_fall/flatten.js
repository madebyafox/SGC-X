...
//mongoimport --collection=ALL_RAW --db=XFALL2017 --file=alfa-bravo-charlie.json
...
//NO FREE RESPONSE BLOCKS IN FALL 2017

// 2.1 Unravel multiple choice blocks (no mouse data) 
db.ALL_BLOCKS.aggregate([
    { $match:{
        "$and":[ 
            { "subject": {$nin: lazies }},
...
// 2.2 Unravel multiple choice blocks (WITH mouse data) 
db.ALL_BLOCKS.aggregate([
    { $match:{
        "$and":[ 
           { "subject": {$nin: lazies }},
