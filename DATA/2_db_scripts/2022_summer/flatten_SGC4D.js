...
db.entries.aggregate([  
    ...
    //set id to subject
    {$set: {_id : "$subject"}},
    {$out: "SGC4D_final_participants"}
  ]);   

//SONA RECONCILIATION LIST
db.SGC4D_final_participants.aggregate([
    {$project:{
        _id:"$subject",
        subject:1,
        exp_id:1,
    ...
    {$out:"SGC4D_final_sona_data"}
]);

//CREATE A COLLECTION OF TEST ITEMS [with mouse data]
db.SGC4D_BY_TRIAL.aggregate([
    ...
    {$out: "SGC4D_final_items_mouse"}
]);

//CREATE A COLLECTION OF TEST ITEMS [no mouse data]
db.SGC4D_final_items_mouse.aggregate([
